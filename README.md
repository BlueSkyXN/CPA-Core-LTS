...
- LTS release tags use the `v*-lts.*` pattern. Push only the exact LTS tag being released; do not use `git push --tags` after fetching upstream.
...
Start with `config.example.yaml` and the SDK docs under `docs/`. Upstream guides may still be useful as historical background, but verify behavior against this LTS repository before applying them.

## Management API

Management endpoints are exposed under `/v0/management` when remote management is enabled. The LTS usage statistics contract includes `/v0/management/usage`, `/v0/management/usage/export`, and `/v0/management/usage/import`; the current handlers live under `internal/api/handlers/management`.
...
Amp routes are implemented under `internal/api/modules/amp`; verify configuration behavior against the `amp-code` section in `config.example.yaml`.
