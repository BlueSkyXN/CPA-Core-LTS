...
func openHostHTTPStream(caller hostCaller, req hostHTTPRequest) (hostHTTPStreamResponse, error) {
	if caller == nil {
		return hostHTTPStreamResponse{}, fmt.Errorf("Copilot host HTTP callback is unavailable")
	}
	raw, err := caller.Call(pluginabi.MethodHostHTTPDoStream, req)
	if err != nil {
		return hostHTTPStreamResponse{}, fmt.Errorf("open Copilot upstream stream")
	}
	var resp hostHTTPStreamResponse
	if json.Unmarshal(raw, &resp) != nil || strings.TrimSpace(resp.StreamID) == "" {
		return hostHTTPStreamResponse{}, fmt.Errorf("invalid Copilot host stream response")
	}
	return resp, nil
}
...
func readHostHTTPStream(caller hostCaller, id string) (hostHTTPStreamReadResponse, error) {
	...
	if err != nil {
		return hostHTTPStreamReadResponse{}, fmt.Errorf("read Copilot upstream stream")
	}
	var resp hostHTTPStreamReadResponse
	if json.Unmarshal(raw, &resp) != nil {
		return resp, fmt.Errorf("invalid Copilot host stream chunk")
	}
	return resp, nil
}
...
func doHostHTTP(caller hostCaller, req hostHTTPRequest) (hostHTTPResponse, error) {
	if caller == nil {
		return hostHTTPResponse{}, fmt.Errorf("Copilot host HTTP callback is unavailable")
	}
	raw, errCall := caller.Call(pluginabi.MethodHostHTTPDo, req)
	if errCall != nil {
		return hostHTTPResponse{}, fmt.Errorf("Copilot host HTTP request failed")
	}
	var response hostHTTPResponse
	if errDecode := json.Unmarshal(raw, &response); errDecode != nil {
		return hostHTTPResponse{}, fmt.Errorf("decode Copilot host HTTP response")
	}
	return response, nil
}
