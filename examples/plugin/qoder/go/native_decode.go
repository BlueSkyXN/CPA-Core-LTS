...
func (d *nativeDecoder) event(kind pluginapi.AgentEventType, payload any) error {
	...
	event := pluginapi.AgentEventV1{SchemaVersion: 1, Type: kind, RequestID: d.projection.requestID, Provider: pluginIdentifier, Sequence: d.projection.sequence + 1, Timestamp: time.Now().UTC(), Payload: raw}
	if d.emit == nil {
...
func (d *nativeDecoder) data(raw []byte) (bool, error) {
	...
	if err := nativeEnvelopeError(obj, raw); err != nil {
		return false, err
	}
	if body, exists := obj["body"]; exists {
		var nested string
		if json.Unmarshal(body, &nested) != nil {
			return false, nativeInvalidResponse()
		}
		var nestedObj map[string]json.RawMessage
		if nested != "[DONE]" && (json.Unmarshal([]byte(nested), &nestedObj) != nil || nestedObj == nil) {
			return false, nativeInvalidResponse()
		}
		// 仅解一层，避免不可信的嵌套造成递归堆栈增长。
		if _, recursive := nestedObj["body"]; recursive {
			return false, nativeInvalidResponse()
		}
		return d.data([]byte(nested))
	}
	usageRaw := obj["usage"]
	if len(usageRaw) == 0 || bytes.Equal(usageRaw, []byte("null")) {
		usageRaw = obj["raw_usage"]
	}
	if len(usageRaw) > 0 {
		...
		merged := mergeNativeUsage(d.projection.usage, usage)
...
func nativeEnvelopeError(obj map[string]json.RawMessage, raw []byte) error {
	status := 0
	_ = json.Unmarshal(obj["statusCodeValue"], &status)
	if status >= 400 {
		body := raw
		var nested string
		if json.Unmarshal(obj["body"], &nested) == nil {
			body = []byte(nested)
		}
		return qoderUpstreamError(status, body)
	}
	if errValue, exists := obj["error"]; exists && !bytes.Equal(errValue, []byte("null")) {
		return qoderUpstreamError(502, raw)
	}
	if code := qoderErrorCode(raw); code != "" && code != "0" && code != "200" {
		return qoderUpstreamError(502, raw)
	}
	return nil
}
...
func mergeNativeUsage(previous pluginapi.AgentUsageV1, usage map[string]any) pluginapi.AgentUsageV1 {
