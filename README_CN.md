...
支持调用方、模型和单条上游 Auth 记录的联合并发/滑动窗口限制、有界排队与按需观察。执行、实时观察和资源采样默认关闭；未写 version 时默认按 schema 3 解释，显式 version 1/2 保留旧语义并标记 legacy-policy。不替代上游额度、Home 或完整 usage。配置保存失败保留最后有效策略，已有流按真实生产端结束释放。配套版本、迁移和回退见 [流控指南](docs/lts/flow-control.md)。
