...
没有 Flow 配置的首次使用者直接得到默认关闭的 V3 草稿，不需要迁移空配置；仅查看或修改其他配置不写入 Flow。新建空策略可以省略 `version`，按 3 解释；已有非空且未指定版本的策略继续按旧语义运行并标记为 legacy，显式 `version: 1/2` 也保留旧语义。建议先经 migration-preview 迁移再保存 `version: 3`。真实 V1/V2 规则和未知内容不会因此被静默升级。
...
| 参数 | 语义与范围 |
|---|---|
| `version` | 新建空策略未填写时默认按 3 解释：单账号、模型复选和首次联合申请。已有非空无版本策略及显式 1/2 保留旧语义，管理接口以 `legacy-policy: true` 标记并建议通过 migration-preview 迁移 |
...
| 方法与路径 | 用途 |
|---|---|
| GET `/flow-control` | schema3能力、期望配置、有效策略、失败说明、引用目录和摘要；期望或已应用策略为非空无版本或显式 1/2 时返回 `legacy-policy: true` |
