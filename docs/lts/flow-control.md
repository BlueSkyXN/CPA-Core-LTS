...
没有 Flow 配置的首次使用者直接得到默认关闭的 V3 草稿，不需要迁移空配置；仅查看或修改其他配置不写入 Flow。`version` 可以省略，省略时按 3 解释；显式 `version: 1/2` 的旧策略继续按旧语义运行并被标记为 legacy，建议先经 migration-preview 迁移再保存 `version: 3`。真实 V1/V2 规则和未知内容不会因此被静默升级。
...
| 参数 | 语义与范围 |
|---|---|
| `version` | 未填写时默认按 3 解释：单账号、模型复选和首次联合申请。显式 1 或 2 保留旧语义继续运行，管理接口以 `legacy-policy: true` 标记并建议通过 migration-preview 迁移 |
...
| 方法与路径 | 用途 |
|---|---|
| GET `/flow-control` | schema3能力、期望配置、有效策略、失败说明、引用目录和摘要；期望或已应用策略为显式 1/2 时返回 `legacy-policy: true` |
