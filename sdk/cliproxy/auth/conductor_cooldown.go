...
func (m *Manager) setConfigSnapshotLocked(cfg *internalconfig.Config) bool {
	...
	homeAliasTTLChanged := homeSessionAliasTTL(previous) != homeSessionAliasTTL(cfg)
...
func (m *Manager) restoreCooldownRecordLocked(record CooldownStateRecord, now time.Time) bool {
	...
	mergeModelState(state, &ModelState{
		...
		UpdatedAt:           updatedAt,
		modelFallbackReason: strings.ToLower(strings.TrimSpace(record.ModelFallbackReason)),
	})
	auth.Generation++
	auth.UpdatedAt = updatedAt
...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth != nil {
		modelKey = modelStateKey(auth.Provider, result.Model)
...
func ensureModelState(auth *Auth, model string) *ModelState {
	...
	model = modelStateKey(provider, model)
	if auth == nil || model == "" {
		return nil
	}
...
func normalizeModelStates(auth *Auth) bool {
	...
	changed := false
	for model, state := range auth.ModelStates {
		modelKey := modelStateKey(auth.Provider, model)
		if modelKey == "" {
...
func mergeModelState(target, source *ModelState) *ModelState {
	...
	merged := ModelState{
		...
		modelFallbackReason: preferred.modelFallbackReason,
		Quota: QuotaState{
	...
	if merged.modelFallbackReason == "" {
		merged.modelFallbackReason = fallback.modelFallbackReason
	}
...
func shouldSkipCredentialCooldown(err *Error) bool {
	...
	if err != nil && err.HTTPStatus == http.StatusBadGateway && isConnectionLifecycleMessage(err.Message) {
		return false
	}
	return isConnectionLifecycleResultError(err)
}
...
func isModelSupportError(err error) bool {
	...
	if statusCodeFromError(err) == http.StatusNotFound && isRequestScopedModelNotFoundMessage(err.Error()) {
		return false
	}
...
func isModelSupportResultError(err *Error) bool {
	if err == nil || err.IsRequestScoped() || isRequestScopedNotFoundResultError(err) {
		return false
	}
	if isExplicitModelNotFoundError(err, "") {
		return true
...
func isRequestScopedNotFoundResultError(err *Error) bool {
	...
	return clienterror.IsItemNotPersisted(err.Message) || isRequestScopedModelNotFoundMessage(err.Message)
}
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool) {
	...
	defer func() {
		...
		// deliberate zero write (disableCooling) still clears the deadline.
