...
func (s *authScheduler) upsertAuthLifecycleLocked(auth *Auth, now time.Time) {
	...
	if s.isStaleScheduledAuth(authID, snapshot.RegistrationEpoch, snapshot.Generation, snapshot.UpdatedAt) {
		return
	}
	s.authGenerations[authID] = scheduledGenerationMeta{
		epoch:      snapshot.RegistrationEpoch,
		generation: snapshot.Generation,
		updatedAt:  snapshot.UpdatedAt,
	}

	providerKey := executorKeyFromAuth(snapshot)
	if providerKey == "" || snapshot.Disabled || snapshot.Status == StatusDisabled {
		s.removeAuthFromProvidersLocked(authID)
		return
	}
	...
	meta := buildScheduledAuthMetaWithModelSet(snapshot, supportedModelSetForAuth(snapshot.ID), regEpoch)
