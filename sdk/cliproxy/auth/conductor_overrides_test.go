...
func TestManager_ShouldRetryAfterError_RespectsAuthRequestRetryOverride(t *testing.T) {
	...
	wait, shouldRetry, terminalErr := m.shouldRetryAfterError(&Error{HTTPStatus: 500, Message: "boom"}, 0, []string{"claude"}, model, maxWait, nil)
	if terminalErr != nil {
		t.Fatalf("terminal error = %v, want nil", terminalErr)
	}
	if shouldRetry {
	...
	_, shouldRetry, terminalErr = m.shouldRetryAfterError(&Error{HTTPStatus: 500, Message: "boom"}, 1, []string{"claude"}, model, maxWait, nil)
	if terminalErr != nil {
		t.Fatalf("terminal error = %v, want nil", terminalErr)
	}
	if shouldRetry {
...
func TestManager_ShouldRetryAfterError_SkipsWrappedHomeConcurrencyBusy(t *testing.T) {
	...
	wait, shouldRetry, terminalErr := m.shouldRetryAfterError(errBusy, 0, []string{"codex"}, "gpt", maxWait, nil)
	if terminalErr != nil {
		t.Fatalf("wrapped Home busy terminal error = %v, want nil", terminalErr)
	}
...
func TestManager_ShouldRetryAfterError_UsesOAuthModelAliasForCooldown(t *testing.T) {
	...
	_, _, maxWait := m.retrySettings()
	wait, shouldRetry, terminalErr := m.shouldRetryAfterError(&Error{HTTPStatus: 429, Message: "quota"}, 0, []string{"kimi"}, routeModel, maxWait, nil)
	if terminalErr != nil {
		t.Fatalf("terminal error = %v, want nil", terminalErr)
	}
	if !shouldRetry {
...
func (e *authFallbackExecutor) ExecuteStream(_ context.Context, auth *Auth, _ cliproxyexecutor.Request, _ cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if chunkErr == nil {
		chunkErr = e.streamTailErrors[auth.ID]
	}
	e.mu.Unlock()
	if returnErr != nil {
...
func TestManager_MarkResult_TransientErrorCooldownConfigured(t *testing.T) {
	prevQuota := quotaCooldownDisabled.Load()
	quotaCooldownDisabled.Store(false)
	prevTransient := transientErrorCooldownSeconds.Load()
	SetTransientErrorCooldownSeconds(30)
	t.Cleanup(func() {
	...
	model := "test-model-transient-configured"
	m.MarkResult(context.Background(), Result{
		AuthID:   auth.ID,
	...
	if state.NextRetryAfter.IsZero() {
		t.Fatal("expected transient error cooldown to be set")
	}
	diff := time.Until(state.NextRetryAfter)
	if diff < 25*time.Second || diff > 35*time.Second {
		t.Fatalf("expected transient error cooldown to be ~30 seconds, got %v", diff)
	}
}

func TestManager_MarkResult_TransientErrorCooldownLegacyDefault(t *testing.T) {
	prevQuota := quotaCooldownDisabled.Load()
	quotaCooldownDisabled.Store(false)
	prevTransient := transientErrorCooldownSeconds.Load()
	...
	model := "test-model-transient-legacy"
	m.MarkResult(context.Background(), Result{
		AuthID:   auth.ID,
		Provider: auth.Provider,
		Model:    model,
...
func TestCountTokensResultErrorScopesOnlyNotFound(t *testing.T) {
	for _, status := range []int{http.StatusUnauthorized, http.StatusTooManyRequests, http.StatusInternalServerError} {
		resultErr := countTokensResultErrorFromError(&Error{HTTPStatus: status, Message: "failure"}, "count-model")
		if resultErr.IsRequestScoped() {
			t.Fatalf("status %d unexpectedly request-scoped", status)
		}
	}
	resultErr := countTokensResultErrorFromError(&Error{HTTPStatus: http.StatusNotFound, Message: "missing count endpoint"}, "count-model")
	if !resultErr.IsRequestScoped() {
		t.Fatalf("404 error = %#v, want request-scoped", resultErr)
	}
	resultErr = countTokensResultErrorFromError(&Error{
		Code:       "model_not_found",
		HTTPStatus: http.StatusNotFound,
		Message:    `{"error":{"code":"model_not_found","message":"model count-model was not found"}}`,
	}, "count-model")
	if resultErr.IsRequestScoped() {
		t.Fatalf("explicit model 404 = %#v, want model-scoped", resultErr)
	}
}
