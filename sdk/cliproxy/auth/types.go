...
// DisableCoolingOverride returns the auth-scoped disable_cooling override when enabled.
// The value is read from metadata key "disable_cooling" (or legacy "disable-cooling").
// False is treated as absent so the global LTS policy still applies.
...
// A negative value preserves the historical LTS behavior and disables retries.
