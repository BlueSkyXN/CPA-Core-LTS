...
func TestManager_ShouldRetryAfterError_RetriesPreHTTPTransportFailure(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			wait, shouldRetry, errRetry := m.shouldRetryAfterError(tc.err, 0, []string{"codex"}, model, 0, nil)
			if errRetry != nil {
				t.Fatalf("shouldRetryAfterError() error = %v", errRetry)
			}
			...
			if tc.want {
				if _, shouldRetry, errRetry = m.shouldRetryAfterError(tc.err, 1, []string{"codex"}, model, 0, nil); errRetry != nil {
					t.Fatalf("second shouldRetryAfterError() error = %v", errRetry)
				}
				if shouldRetry {
