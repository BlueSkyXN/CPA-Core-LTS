...
func TestSessionAffinitySelectorLCPRejectedPreDispatchDoesNotPublishBinding(t *testing.T) {
	selector := NewSessionAffinitySelectorWithConfig(SessionAffinityConfig{
		Fallback: &RoundRobinSelector{},
		TTL:      time.Minute,
	})
	...
	newOptions := func() cliproxyexecutor.Options {
		return cliproxyexecutor.Options{
			SourceFormat:    sdktranslator.FormatOpenAI,
			OriginalRequest: []byte(`{"messages":[{"role":"user","content":"cold LCP admission"}]}`),
			Metadata: map[string]any{
				cliproxyexecutor.CallerScopeMetadataKey: "caller-admission",
			},
		}
	}
	...
	first, errFirst := selector.PickPreDispatch(context.Background(), "openai", "model", firstOpts, auths)
	if errFirst != nil {
		t.Fatalf("first PickPreDispatch() error = %v", errFirst)
	}
	second, errSecond := selector.PickPreDispatch(context.Background(), "openai", "model", secondOpts, auths)
	if errSecond != nil {
		t.Fatalf("second PickPreDispatch() error = %v", errSecond)
	}
	if first.ID == second.ID {
		t.Fatalf("concurrent cold LCP selections = %q and %q, want independent fallback picks before admission", first.ID, second.ID)
	}

	namespace := lcpAffinityNamespace("openai", "model", firstOpts.Metadata)
	fingerprints, minPrefixLength := lcpFingerprintsFromMetadata(firstOpts.Metadata)
	if match, ok := selector.matcher.MatchFingerprints(namespace, fingerprints, minPrefixLength); ok {
		t.Fatalf("LCP binding published before admission: %#v", match)
	}

	selector.ReleasePreDispatchSelection(first.ID, firstOpts)
	selector.ReleasePreDispatchSelection(second.ID, secondOpts)
	if match, ok := selector.matcher.MatchFingerprints(namespace, fingerprints, minPrefixLength); ok {
		t.Fatalf("rejected LCP selection remained published: %#v", match)
	}

	committedOpts := newOptions()
	committed, errCommitted := selector.PickPreDispatch(context.Background(), "openai", "model", committedOpts, auths)
	if errCommitted != nil {
		t.Fatalf("committed PickPreDispatch() error = %v", errCommitted)
	}
	selector.CommitPreDispatchSelection(committed.ID, committedOpts)
	committedFingerprints, committedMinPrefixLength := lcpFingerprintsFromMetadata(committedOpts.Metadata)
	match, ok := selector.matcher.MatchFingerprints(namespace, committedFingerprints, committedMinPrefixLength)
	if !ok || match.AuthID != committed.ID {
		t.Fatalf("LCP binding after admission commit = %#v, %v; want auth %q", match, ok, committed.ID)
	}
	if got := committedOpts.Metadata[cliproxyexecutor.LCPAffinitySessionIDMetadataKey]; got != match.SessionID {
		t.Fatalf("committed LCP session metadata = %#v, want %q", got, match.SessionID)
	}
}
