...
func TestDisableCoolingOverrideTreatsFalseAsInherited(t *testing.T) {
	tests := []struct {
		name        string
		auth        *Auth
		want        bool
		...
		{name: "canonical false", auth: &Auth{Metadata: map[string]any{"disable_cooling": false}}},
		{name: "legacy false", auth: &Auth{Metadata: map[string]any{"disable-cooling": false}}},
		{name: "string false", auth: &Auth{Metadata: map[string]any{"disable_cooling": "false"}}},
