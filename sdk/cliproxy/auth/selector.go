...
func modelStateKey(provider, model string) string {
	model = strings.TrimSpace(model)
...
func modelStateMatchesRoute(provider, stateModel, routeModel string) bool {
	stateModel = strings.TrimSpace(stateModel)
	routeModel = strings.TrimSpace(routeModel)
	if stateModel == "" || routeModel == "" {
		return false
	}
	if !strings.EqualFold(strings.TrimSpace(provider), "codex") {
		return canonicalModelKey(stateModel) == canonicalModelKey(routeModel)
	}
	stateBase := canonicalModelKey(stateModel)
	if !strings.EqualFold(stateModel, stateBase) {
		return strings.EqualFold(stateModel, routeModel)
	}
	return strings.EqualFold(stateBase, canonicalModelKey(routeModel))
}
	...
	if model != "" {
		if len(auth.ModelStates) > 0 {
			...
			for stateModel, state := range auth.ModelStates {
				if state == nil || !modelStateMatchesRoute(auth.Provider, stateModel, model) {
					continue
				}
				matched = true
...
// Private host execution and canonical Codex metadata remain authoritative for
// LTS continuity. Native Claude Code, Codex, Amp, OpenCode, pi, and request-body
// signals are then authoritative. Requests without those signals use the Merkle LCP
...
func (s *SessionAffinitySelector) pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, deferBinding bool) (*Auth, error) {
	...
	if explicitID == "" {
		if auth, handled, errLCP := s.pickLCP(ctx, provider, model, opts, auths, entry, deferBinding); handled || errLCP != nil {
			return auth, errLCP
		}
			...
			for _, auth := range available {
				if auth.ID == cachedAuthID {
					if !isSubagent || s.subagentAffinity {
						bind(auth.ID, !isSubagent)
						if isFork {
							entry.Infof("session-affinity: fork cache hit | session=%s parent=%s auth_index=%s provider=%s model=%s", sessionLogIdentity(primaryID), sessionLogIdentity(fallbackID), sessionAffinityAuthLogID(auth), provider, model)
						} else {
	...
	if isFork && fallbackID != "" {
		entry.Infof("session-affinity: fork bound to new auth | session=%s parent=%s auth_index=%s provider=%s model=%s", sessionLogIdentity(primaryID), sessionLogIdentity(fallbackID), sessionAffinityAuthLogID(auth), provider, model)
	} else {
...
func (s *SessionAffinitySelector) CommitPreDispatchSelection(authID string, opts cliproxyexecutor.Options) {
	...
	if s.cache != nil && len(marker.aliases) > 0 {
...
func (s *SessionAffinitySelector) pickLCP(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth, entry *log.Entry, deferBinding bool) (*Auth, bool, error) {
...
//  2. pre-auth canonical Codex session metadata
//  3. native Claude Code and Codex session signals
//  4. X-Session-ID, X-Amp-Thread-Id, X-Session-Affinity, and X-Client-Request-Id
//  5. session_id / sessionId
//  6. prompt_cache_key, with conversation / conversation.id as an alias
//  7. metadata.user_id and conversation_id legacy body fields
//  8. stable context-derived session identity
//  9. stable hash from initial message content
...
func extractExplicitSessionIDs(headers http.Header, payload []byte, metadata map[string]any) (string, string) {
	...
	if sessionID := contextStringValue(metadata[codexCanonicalSessionMetadataKey]); sessionID != "" {
