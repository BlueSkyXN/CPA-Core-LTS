...
func (m *Manager) executeRequestUncontrolled(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	if isRequestStopError(err) {
		return cliproxyexecutor.Response{}, unwrapExecutionBoundaryError(err)
	}
	resp, err = m.executeCodexModelFallback(ctx, providers, req, opts, err)
	return resp, unwrapExecutionBoundaryError(err)
}
...
func (m *Manager) executeStreamRequestUncontrolled(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if isRequestStopError(err) {
		return nil, unwrapExecutionBoundaryError(err)
	}
	...
	if isHomeRetryRoundExhausted(err) {
		...
		if errors.As(err, &homeBootstrapErr) && homeBootstrapErr != nil {
			return streamErrorResult(SafeResponseHeaders(err), unwrapExecutionBoundaryError(err)), nil
		}
		return nil, unwrapExecutionBoundaryError(err)
	}
...
// applyRequestAfterAuthInterceptor preserves the upstream package-level helper
// used by auth package integrations that do not have a Manager instance.
func applyRequestAfterAuthInterceptor(ctx context.Context, executor ProviderExecutor, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, requestedModel string) (cliproxyexecutor.Request, cliproxyexecutor.Options, error) {
	return applyRequestAfterAuthInterceptorWithManager(nil, ctx, executor, nil, provider, req, opts, requestedModel)
}
...
func (m *Manager) applyRequestAfterAuthInterceptor(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, requestedModel string) (cliproxyexecutor.Request, cliproxyexecutor.Options, error) {
	return applyRequestAfterAuthInterceptorWithManager(m, ctx, executor, auth, provider, req, opts, requestedModel)
}

func applyRequestAfterAuthInterceptorWithManager(m *Manager, ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, requestedModel string) (cliproxyexecutor.Request, cliproxyexecutor.Options, error) {
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		var provider string
		var errPick error
		if m.hasLegacyHomeRuntimeAuthForRequest(ctx, opts) {
			var ok bool
			auth, executor, provider, ok = m.homeRuntimeAuthByID(homeExecutionSessionIDFromMetadata(opts.Metadata), pinnedAuthIDFromMetadata(opts.Metadata))
			if !ok {
				errPick = &Error{Code: "home_unavailable", Message: "home runtime auth unavailable", HTTPStatus: http.StatusServiceUnavailable}
			}
		} else {
		...
		if errPrepare != nil {
			...
			result := resultForAuthWithOptions(auth, provider, stateModel, false, resultErrorFromError(errPrepare), pickOpts)
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, homeRetryLimit *int, retryRound int, defaultRequestRetry int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		if !allowed {
			if selection != nil {
				if errEnd := m.endHomeSelectionBeforeRedispatch(ctx, selection, "continuity_observation_pending"); errEnd != nil {
					return nil, errEnd
				}
				homeAuthCount++
			}
		...
		debugLogAuthSelection(entry, auth, provider, routeModel)
		if selection != nil {
			if errRuntimeAuth := m.bindHomeSelectionRuntimeAuth(attemptCtx, opts, selection); errRuntimeAuth != nil {
		...
		if selection != nil {
			var errBind error
			execCtx, releaseAttempt, errBind = homeExecutionAttemptContext(attemptCtx, selection)
			if errBind != nil {
		...
		if selection != nil {
			...
			if m.retainHomeWebsocketSelection(execCtx, opts, routeModel, selection) {
				return wrapHomeStream(execCtx, streamResult, nil, releaseAttempt), nil
			}
			return wrapHomeStream(execCtx, streamResult, selection, releaseAttempt), nil
		}
		streamHandedOff = true
...
func (m *Manager) PrepareRequestAuth(ctx context.Context, preparer RequestAuthPreparer, auth *Auth) (*Auth, error) {
	...
	saved, applied, errUpdate := m.updateFromAsyncWithMode(ctx, base, updated, updateModePrepare)
	if errUpdate != nil {
