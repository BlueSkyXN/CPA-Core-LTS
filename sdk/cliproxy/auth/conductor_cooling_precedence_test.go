...
func TestManagerMarkResultUsesCredentialCoolingPrecedence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "credential false inherits global true", globalDisable: true, credential: &enabled},
		{name: "unset inherits global true", globalDisable: true},
		{name: "unset inherits global false", wantCooldown: true},
		{name: "provider false inherits global true", globalDisable: true, providerOverride: &enabled},
		{name: "provider true overrides global false", providerOverride: &disabled},
		{name: "credential false inherits provider true", credential: &enabled, providerOverride: &disabled},
