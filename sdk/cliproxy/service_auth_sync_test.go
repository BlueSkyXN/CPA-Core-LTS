...
func TestRuntimeAuthSyncHook_SynchronousModelAndSchedulerRestoration(t *testing.T) {
	...
	manager.MarkResult(context.Background(), coreauth.Result{
		AuthID: authID, Provider: "codex", Model: "gpt-6-astra", CredentialScope: true,
		RetryAfter: &retryAfter, Error: &coreauth.Error{HTTPStatus: http.StatusTooManyRequests, Code: "usage_limit_reached"},
	})
	quotaAuth, _ := manager.GetByID(authID)
