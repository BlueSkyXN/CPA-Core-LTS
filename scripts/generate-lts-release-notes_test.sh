...
git tag -a v1-lts-0.0.1 \
  ...
  -m 'Companion-Panel: v1-lts-0.0.1'
...
git tag -a v1-lts-0.0.2 \
  ...
  -m 'Companion-Panel: v1-lts-0.0.2'
...
assert_contains "$output_file" 'CPA-Panel-LTS：[v1-lts-0.0.2]'
assert_contains "$output_file" '## What'"'"'s Changed'
assert_not_contains "$output_file" '## 本版要点'
assert_contains "$title_file" 'v1-lts-0.0.2 — Core LTS: explicit summary survives release ordering'
assert_contains "$output_file" 'sky-cpa-core-lts_<version>_<os>_<arch>'
assert_contains "$output_file" 'sky-cpa-core-lts_<version>_linux_<arch>_no-plugin.tar.gz'
assert_not_contains "$output_file" 'CLIProxyAPI_<version>_'
...
assert_contains "$fallback_file" 'CPA-Core-LTS/compare/v1-lts-0.0.1...v1-lts-0.0.2'
