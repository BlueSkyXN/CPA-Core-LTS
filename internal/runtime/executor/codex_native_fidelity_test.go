			...
			for _, buffering := range []bool{false, true} {
				t.Run(fmt.Sprintf("%s/lite=%s/buffering=%t", transport, lite, buffering), func(t *testing.T) {
					...
					if transport == "websocket" {
						...
						if native && alias != "" {
							t.Errorf("native request leaked legacy session alias = %q", alias)
						}
						if canonical := codexSessionHeaderValue(upstreamHeaders); canonical == "" {
							t.Error("LTS canonical websocket session header is empty")
						}
					}
