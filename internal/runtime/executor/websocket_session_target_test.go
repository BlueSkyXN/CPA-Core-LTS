...
func ensureCodexWebsocketConnForTest(executor *CodexWebsocketsExecutor) func(context.Context, *cliproxyauth.Auth, *codexWebsocketSession, string, string, http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error) {
	return func(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID, wsURL string, headers http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error) {
		key := newCodexWebsocketConnectionKey(authID, wsURL, "gpt-5-codex", [32]byte{})
		connection, closer, resp, err := executor.ensureUpstreamConn(ctx, auth, sess, key, headers)
		return connection.conn, closer, resp, err
	}
}
			...
			run: func(t *testing.T, baseURL string) (func(cliproxyexecutor.Options) error, *codexWebsocketSession) {
				...
				return func(runOpts cliproxyexecutor.Options) error {
					if !primed {
						...
						connectionKey := newCodexWebsocketConnectionKey(auth.ID, wsURL, "gpt-5-codex", codexModelHeaderProfile{}.digest)
						connection, _, _, errEnsure := executor.ensureUpstreamConn(context.Background(), auth, executor.getOrCreateSession("retry-bind"), connectionKey, http.Header{})
						if errEnsure != nil {
							return errEnsure
						}
						if errDeadline := connection.conn.SetWriteDeadline(time.Now().Add(-time.Second)); errDeadline != nil {
							return errDeadline
						}
						primed = true
					}
...
func TestWebsocketSessionCloseEndsRetainedLifecycleOnce(t *testing.T) {
	...
	conn := ensureWebsocketTargetConn(t, ensureCodexWebsocketConnForTest(exec), auth, sess, auth.ID, wsURL)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			case "Codex":
				exec := NewCodexWebsocketsExecutor(&config.Config{})
				exec.store = &codexWebsocketSessionStore{sessions: make(map[string]*codexWebsocketSession)}
				ensure = ensureCodexWebsocketConnForTest(exec)
	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(sess *codexWebsocketSession, conn *websocket.Conn, lifecycle *trackedWebsocketLifecycle) {
				...
				sess.connMu.Unlock()
				exec.invalidateUpstreamConn(sess, connection, "invalidated", nil)
...
func TestWebsocketExecutorsReconnectWhenSessionTargetChanges(t *testing.T) {
	t.Run("Codex", func(t *testing.T) {
		...
		testWebsocketExecutorReconnectsWhenSessionTargetChanges(
			...
			func(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error) {
				connection, closer, resp, errEnsure := exec.ensureUpstreamConn(ctx, auth, sess, newCodexWebsocketConnectionKey(authID, wsURL, "gpt-5.4", [32]byte{}), headers)
				return connection.conn, closer, resp, errEnsure
			},
	...
	tests := []struct {
	...
	}{
		{
			...
			newExecutor: func() (cliproxyauth.ProviderExecutor, func(string) *codexWebsocketSession, func(context.Context, *cliproxyauth.Auth, *codexWebsocketSession, string, string, http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error)) {
				...
				executor.store = &codexWebsocketSessionStore{sessions: make(map[string]*codexWebsocketSession)}
				return executor, executor.getOrCreateSession, ensureCodexWebsocketConnForTest(executor)
			},
		},
		{
			name:     "xAI",
	...
	tests := []struct {
	...
	}{
		{
			...
			ensureConn: func(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID, wsURL string, headers http.Header) (*websocket.Conn, *websocketConnectionCloser, *http.Response, error) {
				executor := NewCodexWebsocketsExecutor(&config.Config{})
				return ensureCodexWebsocketConnForTest(executor)(ctx, auth, sess, authID, wsURL, headers)
			},
		},
		{
