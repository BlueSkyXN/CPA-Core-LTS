...
func applyCodexIdentityConfuseHeaders(headers http.Header, state *codexIdentityConfuseState) {
	...
	if headerValueCaseInsensitive(headers, "session_id") != "" {
		updateExistingHeaderCasePreserved(headers, "session_id", state.promptCacheKey)
	}
	if headerValueCaseInsensitive(headers, "Conversation_id") != "" {
		updateExistingHeaderCasePreserved(headers, "Conversation_id", state.promptCacheKey)
	}
...
func updateExistingHeaderCasePreserved(headers http.Header, key string, value string) {
	if headers == nil {
		return
	}
