...
func (r *UsageReporter) ObserveResponseModel(payload []byte) {
	...
	// Preserve the LTS upstream_model field as the first observed served model.
	// Providers that only know their mapped target still populate it through
	// SetUpstreamModel before any response model is available.
	r.upstreamModelMu.Lock()
	if r.upstreamModel == "" {
		r.upstreamModel = served
	}
...
func (r *UsageReporter) UpstreamModel() string {
	...
	r.upstreamModelMu.RLock()
	defer r.upstreamModelMu.RUnlock()
	return r.expectedUpstreamModel
}
...
// sumNonNegativeUsageTokens returns false instead of wrapping when the token
// total cannot be represented as a non-negative int64.
func sumNonNegativeUsageTokens(tokens ...int64) (int64, bool) {
...
func (r *UsageReporter) buildRecordForModel(model string, detail usage.Detail, failed bool, fail usage.Failure) usage.Record {
	...
	// Additional-model records describe a side model (for example image tool
	// usage) that the upstream response model does not identify.
	responseModel := ""
	if model == r.model {
...
func parseOpenAIStyleUsageNode(usageNode gjson.Result) usage.Detail {
	...
	inputTokens, _ := parseStrictUsageTokenCount(inputNode)
	outputTokens, _ := parseOptionalStrictUsageTokenCount(outputNode)
	totalTokens, _ := parseOptionalStrictUsageTokenCount(usageNode.Get("total_tokens"))
	detail := usage.Detail{
	...
	cachedTokens, _ := parseOptionalStrictUsageTokenCount(cached)
	...
	cacheCreationTokens, okCacheCreation := parseOptionalStrictUsageTokenCount(cacheCreation)
	...
	if reasoning.Exists() {
		var okReasoning bool
		detail.ReasoningTokens, okReasoning = parseOptionalStrictUsageTokenCount(reasoning)
		if !okReasoning {
			return usage.Detail{}
		}
	}
	...
	if !outputNode.Exists() && detail.OutputTokens == 0 && detail.ReasoningTokens > 0 {
		detail.OutputTokens = detail.ReasoningTokens
	}
	detail = normalizeUsageInputTokenCategories(detail)
...
func parseClaudeUsageNode(usageNode gjson.Result) usage.Detail {
	...
	inputTokenCount, _ := parseStrictUsageTokenCount(inputTokens)
	...
	canonicalInput, ok := sumNonNegativeUsageTokens(inputTokenCount, cacheReadTokens, cacheCreationTokens)
	if !ok {
		// Claude reports cache categories separately. Keep the original input
		// count when their canonical total would overflow, but clear categories
		// and total because neither aggregate can be proven.
		detail.InputTokens = inputTokenCount
		detail.CachedTokens = 0
		detail.CacheReadTokens = 0
		detail.CacheCreationTokens = 0
	...
	detail.InputTokens = canonicalInput
	detail.TotalTokens, _ = sumNonNegativeUsageTokens(detail.InputTokens, detail.OutputTokens)
	detail = normalizeUsageInputTokenCategories(detail)
...
func parseGeminiFamilyUsageDetail(node gjson.Result) usage.Detail {
	...
	inputTokenCount, _ := parseStrictUsageTokenCount(inputTokens)
	toolUseTokens, okToolUse := parseOptionalStrictUsageTokenCount(firstExistingUsageNode(node, "toolUsePromptTokenCount", "tool_use_prompt_token_count"))
	canonicalInput, okInput := sumNonNegativeUsageTokens(inputTokenCount, toolUseTokens)
	cachedTokens, _ := parseOptionalStrictUsageTokenCount(node.Get("cachedContentTokenCount"))
	outputTokens, _ := parseOptionalStrictUsageTokenCount(node.Get("candidatesTokenCount"))
	reasoningTokens, _ := parseOptionalStrictUsageTokenCount(node.Get("thoughtsTokenCount"))
	totalTokens, _ := parseOptionalStrictUsageTokenCount(node.Get("totalTokenCount"))
	detail := usage.Detail{
	...
	if detail.TotalTokens == 0 {
		var okTotal bool
		detail.TotalTokens, okTotal = sumNonNegativeUsageTokens(detail.InputTokens, detail.OutputTokens, detail.ReasoningTokens)
		if !okTotal {
			detail.TotalTokens = 0
			detail.TokenBreakdown = invalidUsageTokenBreakdown(0)
			return detail
...
func parseInteractionsUsageDetail(node gjson.Result) usage.Detail {
	...
	inputAlreadyIncludesCache := !node.Get("input_tokens").Exists() && !node.Get("prompt_tokens").Exists() && node.Get("total_input_tokens").Exists()
	...
	inputTokenCount, _ := parseStrictUsageTokenCount(inputTokens)
	toolUseTokenCount, okToolUse := parseOptionalStrictUsageTokenCount(firstExistingUsageNode(node, "tool_use_tokens", "total_tool_use_tokens", "toolUseTokens", "totalToolUseTokens"))
	cacheReadTokenCount, _ := parseOptionalStrictUsageTokenCount(cacheRead)
	cachedTokenCount, _ := parseOptionalStrictUsageTokenCount(cachedTokens)
	cacheCreationTokenCount, _ := parseOptionalStrictUsageTokenCount(cacheCreation)
	outputTokenCount, _ := parseOptionalStrictUsageTokenCount(firstExistingUsageNode(node, "output_tokens", "completion_tokens", "total_output_tokens"))
	reasoningTokenCount, _ := parseOptionalStrictUsageTokenCount(firstExistingUsageNode(node, "reasoning_tokens", "thoughtsTokenCount", "total_thought_tokens"))
	totalTokenCount, _ := parseOptionalStrictUsageTokenCount(firstExistingUsageNode(node, "total_tokens", "totalTokenCount"))
	inputTokenCount, okInput := sumNonNegativeUsageTokens(inputTokenCount, toolUseTokenCount)
	detail := usage.Detail{
	...
	// Interactions reports cache read/write categories independently from its
	// input count. A source that explicitly calls the field total_input_tokens
	// has already included them; ordinary input_tokens/prompt_tokens has not.
	if !inputAlreadyIncludesCache {
		canonicalInput := inputTokenCount
		var ok bool
		if cacheRead.Exists() {
			canonicalInput, ok = sumNonNegativeUsageTokens(canonicalInput, detail.CacheReadTokens)
		} else {
			ok = true
		}
		if ok {
			canonicalInput, ok = sumNonNegativeUsageTokens(canonicalInput, detail.CacheCreationTokens)
		}
		if !ok {
			detail.InputTokens = inputTokenCount
			detail.CachedTokens = 0
