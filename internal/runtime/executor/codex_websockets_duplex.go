...
func (e *CodexWebsocketsExecutor) streamCodexDuplex(
	...
	sess *codexWebsocketSession, connection codexWebsocketConnectionRef, readCh chan codexWebsocketRead, requestSignal *codexWebsocketRequestSignal,
	...
	allPrepared := []*codexWebsocketPrepared{initial}
	...
	go func() {
		...
		processCreatePayload := func(payload []byte) bool {
			...
			if prepared.optimizeMultiAgentV2 || prepared.multiAgentV2Conflict {
				sess.setMultiAgentV2Optimized(connection, prepared.optimizeMultiAgentV2 && !prepared.multiAgentV2Conflict)
			}
	...
	go func() {
		...
		defer func() {
			...
			e.invalidateUpstreamConnWithoutDisconnectNotify(sess, connection, "duplex_closed", nil)
			<-writerDone
			sess.clearActive(conn, readCh)
			unlock()
			metadataMu.Lock()
			preparedRequests := append([]*codexWebsocketPrepared(nil), allPrepared...)
			metadataMu.Unlock()
			for _, prepared := range preparedRequests {
				if prepared != nil {
					prepared.identityState.affinity.close()
				}
			}
		}()
		...
		for {
			kind, payload, errRead := readCodexWebsocketMessage(streamCtx, sess, connection, readCh, requestSignal)
			if errRead != nil {
				select {
			...
			restoreMultiAgent := !eventPrepared.multiAgentV2Conflict && (eventPrepared.optimizeMultiAgentV2 || sess.isMultiAgentV2Optimized(connection))
			...
			if eventType == "response.completed" || eventType == "response.done" || eventType == "response.incomplete" {
				...
				if eventType != "response.incomplete" {
					eventPrepared.identityState.affinity.complete(payload)
