...
// A non-overload terminal failure remains an in-stream error even when the selected downstream
// format renders the held handshake frames as no payload. The conductor may still decide that an
// uncommitted stream can be retried; the executor must preserve the ordinary streaming boundary.
func TestCodexExecutor_BootstrapBuffering_InStreamDeliveryNeedsRenderedFrames(t *testing.T) {
	server := codexSSEServer(codexCreatedEvent, codexInProgressEvent, codexOutputAddedEvent, codexInvalidEvent)
	defer server.Close()
...
// A non-overload terminal API error keeps its in-stream delivery: the buffered frames are flushed
// and the error arrives as a chunk rather than burning another credential. Zero-output
// response.incomplete is intentionally excluded because the LTS contract classifies it as a
// request-scoped empty response before the stream is committed.
func TestCodexExecutor_BootstrapBuffering_NonOverloadTerminalStaysInStream(t *testing.T) {
	cases := []struct{ name, tail string }{
		{"invalid request", codexInvalidEvent},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
