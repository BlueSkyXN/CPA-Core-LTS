...
func TestCodexExecutorIsCompatConvertsAgentMessage(t *testing.T) {
	...
	payload := bytes.Replace(codexSpawnAgentTestPayload(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantRoleExists {
				...
				if message.Get("content.#").Int() != 1 || message.Get("content.0.type").String() != "input_text" || message.Get("content.0.text").String() != "Payload:\n\ndelegated task" {
