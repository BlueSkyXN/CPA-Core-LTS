...
// client-supplied OAuth tool name and whether a rename is needed. Besides the
// static built-in map, it normalizes single-underscore MCP names used by some
// third-party clients (`mcp_<tool>`) into Claude Code's double-underscore form
// (`mcp__<tool>`), which Anthropic treats as first-party OAuth traffic.
