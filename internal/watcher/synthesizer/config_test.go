...
func TestConfigSynthesizer_DisableCoolingMetadataFromConfig(t *testing.T) {
	...
	ctx := &SynthesisContext{
		Config: &config.Config{
			GeminiKey: []config.GeminiKey{
				{APIKey: "gemini-key", DisableCooling: boolPointer(true)},
			},
			ClaudeKey: []config.ClaudeKey{
				{APIKey: "claude-key", DisableCooling: boolPointer(true)},
			},
			CodexKey: []config.CodexKey{
				{APIKey: "codex-key", DisableCooling: boolPointer(true)},
			},
	...
	for _, provider := range []string{"gemini", "claude", "codex", "openai-compatible-openrouter"} {
...
func TestConfigSynthesizer_DisableCoolingFalseOmitsMetadata(t *testing.T) {
	...
	ctx := &SynthesisContext{
		Config: &config.Config{
			GeminiKey: []config.GeminiKey{
				{APIKey: "gemini-key", DisableCooling: boolPointer(false)},
			},
		},
		Now:         time.Now(),
		IDGenerator: NewStableIDGenerator(),
