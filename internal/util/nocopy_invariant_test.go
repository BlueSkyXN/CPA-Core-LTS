...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	"internal/usage/query.go":                               {1, "copies immutable RequestDetail structs into a query-local fixed array; no payload bytes or no-copy GJSON backing buffers are overwritten"},
	...
	"sdk/cliproxy/flowcontrol/engine.go":                    {1, "shifts []*waiter to remove a queue entry; elements are internal queue records, never payload buffers"},
}
