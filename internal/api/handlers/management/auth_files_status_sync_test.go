...
func TestPatchAuthFileStatusHookErrorReturns500(t *testing.T) {
	...
	if strings.Contains(rec.Body.String(), "simulated sync hook failure") || !strings.Contains(rec.Body.String(), "failed to synchronize auth runtime") {
		t.Fatal("runtime sync failure must be reported without exposing hook error details")
	}
}
...
func TestPatchPluginVirtualSourceStatusHookErrorReturnsError(t *testing.T) {
	...
	if strings.Contains(rec.Body.String(), "plugin sync failed") || !strings.Contains(rec.Body.String(), "failed to synchronize plugin virtual auth") {
		t.Fatal("plugin runtime sync failure must not expose hook error details")
	}
}
