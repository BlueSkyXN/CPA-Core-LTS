...
func TestListAuthFilesCooldownsSnapshotPrunesOnlyExpiredLTSState(t *testing.T) {
	...
	for _, pair := range [][2]*coreauth.Auth{{beforeA, afterA}, {beforeB, afterB}} {
		before, after := pair[0], pair[1]
		if before == nil || after == nil {
			t.Fatalf("missing auth around cooldown snapshot: before=%#v after=%#v", before, after)
		}
		if !reflect.DeepEqual(before.ModelStates["model-a"], after.ModelStates["model-a"]) {
			t.Fatalf("active model cooldown changed: before=%#v after=%#v", before.ModelStates["model-a"], after.ModelStates["model-a"])
		}
		expired := after.ModelStates["expired"]
		if expired == nil || expired.Unavailable || expired.Status == coreauth.StatusError || !expired.NextRetryAfter.IsZero() || expired.Quota.Exceeded || !expired.Quota.NextRecoverAt.IsZero() {
			t.Fatalf("expired LTS cooldown was not pruned: %#v", expired)
		}
	}
}
