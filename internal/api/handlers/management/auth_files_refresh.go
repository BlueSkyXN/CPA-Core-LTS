...
func (h *Handler) RefreshAuthFiles(c *gin.Context) {
	...
	if req.All {
		...
		for i := range results {
			if !results[i].Success {
				results[i].Error = "auth refresh failed"
			}
		}
	...
	refreshed.EnsureIndex()
	// Refresh is a status operation, not the explicit credential download route.
	c.JSON(http.StatusOK, gin.H{
		"ok": true,
		"auth": gin.H{
			"id":                refreshed.ID,
			"auth_index":        refreshed.Index,
