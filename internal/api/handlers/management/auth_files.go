...
func reconcileAuthFileCooldownState(auth *coreauth.Auth, now time.Time) (unavailable bool, status coreauth.Status, statusMessage string, nextRetry time.Time) {
	...
	// ListAuthFiles prunes elapsed resettable state before building entries.
	// If an unavailable error remains while recorded model state is otherwise
	// schedulable, it is an independent credential failure deliberately kept by
	// the auth manager rather than a stale aggregate to promote to active.
	if unavailable && status == coreauth.StatusError && hadCooldown && hasSchedulableModels && !allSchedulableBlocked {
		if !nextRetry.IsZero() && !nextRetry.After(now) {
			nextRetry = time.Time{}
		}
		return true, status, statusMessage, nextRetry
	}
