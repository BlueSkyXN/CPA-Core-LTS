...
func TestSaveTokenRecord_PreservesMatchingLegacyClaudeCredentialIdentity(t *testing.T) {
	authDir := t.TempDir()
	...
	if savedPath != legacyPath {
		t.Fatalf("savedPath = %s, want %s", savedPath, legacyPath)
	}
	if _, errStat := os.Stat(targetPath); !os.IsNotExist(errStat) {
		t.Fatalf("unexpected canonical copy exists or stat failed: %v", errStat)
	}

	savedRaw, errRead := os.ReadFile(savedPath)
	if errRead != nil {
