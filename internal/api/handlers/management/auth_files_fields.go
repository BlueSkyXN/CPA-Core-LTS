...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	if coreauth.IsPluginVirtualAuth(targetAuth) {
		...
		if errHook := h.invokePostAuthPersistHooks(ctx, hookAuths); errHook != nil {
			log.Error("post-auth persist hook failed for plugin virtual source status update")
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to synchronize plugin virtual auth"})
			return
		}
...
func decodeAuthFileRequestRetryPatch(fields map[string]json.RawMessage) (authFileRequestRetryPatch, error) {
	...
	if normalized < 0 {
		// LTS compatibility: config and persisted auth metadata have always
		// treated a negative per-auth value as an explicit retry disable.
		normalized = 0
	}
