...
func TestRefreshAuthFiles_AllAndSpecific(t *testing.T) {
	...
	for _, secret := range []string{"refreshed-token", "refresh-token", "access_token", "refresh_token", "metadata", "attributes"} {
		if strings.Contains(wSingle.Body.String(), secret) {
			t.Fatal("refresh status response exposed credential metadata")
		}
	}
	...
	if err := json.Unmarshal(wSingle.Body.Bytes(), &statusResponse); err != nil || statusResponse.Auth.ID == "" || statusResponse.Auth.AuthIndex == "" {
		t.Fatal("refresh response lost its non-secret identity")
	}
...
func TestRefreshAuthFilesErrorsDoNotExposeProviderCredentials(t *testing.T) {
	gin.SetMode(gin.TestMode)
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(&refreshRecordExecutor{
		provider:   "antigravity",
		refreshErr: errors.New("provider rejected fixture-private-refresh-value"),
	})
	_, err := manager.Register(context.Background(), &coreauth.Auth{
		ID: "refresh-error.json", Provider: "antigravity", Status: coreauth.StatusActive,
		Metadata: map[string]any{"type": "antigravity", "refresh_token": "fixture-refresh", "expired": "2020-01-01T00:00:00Z"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	engine := gin.New()
	engine.POST("/auth-files/refresh", h.RefreshAuthFiles)
	for _, query := range []string{"?name=refresh-error.json", "?all=true"} {
		w := httptest.NewRecorder()
		engine.ServeHTTP(w, httptest.NewRequest(http.MethodPost, "/auth-files/refresh"+query, nil))
		if strings.Contains(w.Body.String(), "fixture-private-refresh-value") || !strings.Contains(w.Body.String(), "auth refresh failed") {
			t.Fatal("refresh error response must contain a bounded failure without provider payload")
		}
	}
}
