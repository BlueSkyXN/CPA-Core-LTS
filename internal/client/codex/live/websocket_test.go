...
func TestHandleDirectWebsocketPublishesZeroTokenUsageAndMarksLocalAuthResult(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstreamServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		upgrader := websocket.Upgrader{CheckOrigin: func(*http.Request) bool { return true }}
		connection, errUpgrade := upgrader.Upgrade(writer, request, nil)
	...
	const pluginName = "codex-realtime-websocket-success-test"
	const authID = "codex-realtime-websocket-usage-oauth"
	usage.RegisterNamedPlugin(pluginName, &captureLiveUsagePlugin{records: records, authID: authID, model: "gpt-live-1-codex"})
	...
	if record.Failed || record.AuthID != credential.ID || record.Detail.TotalTokens != 0 {
		t.Fatalf("Codex direct Realtime usage record = %+v", record)
	}
	...
	if !ok || updated.Success != 1 || updated.Failed != 0 {
		t.Fatalf("local auth outcome = %#v, ok=%t; want one successful WebSocket result", updated, ok)
	}
}
