...
func (p *homeUnauthorizedUsageCapture) HandleUsage(_ context.Context, record usage.Record) {
...
func (p *homeUnauthorizedUsageCapture) wait(t *testing.T) usage.Record {
	t.Helper()
	select {
...
func (noopHomeUnauthorizedUsagePlugin) HandleUsage(context.Context, usage.Record) {}

func registerHomeUnauthorizedUsageCapture(t *testing.T, name, authID string) *homeUnauthorizedUsageCapture {
	t.Helper()
	capture := &homeUnauthorizedUsageCapture{authID: authID, records: make(chan usage.Record, 1)}
	usage.RegisterNamedPlugin(name, capture)
	t.Cleanup(func() {
		usage.RegisterNamedPlugin(name, noopHomeUnauthorizedUsagePlugin{})
	})
	return capture
}
...
func TestHandlerForwardsUnauthorizedHomeResponseWithoutRefresh(t *testing.T) {
	...
	attempt := waitForLiveUsageRecord(t, records)
	if !attempt.Failed || attempt.Fail.StatusCode != http.StatusUnauthorized || attempt.Fail.Body != upstreamError {
		t.Fatalf("Home Live usage attempt = %+v, want one failed 401 with upstream body", attempt)
	}
	assertNoAdditionalLiveUsageRecord(t, records)
	if errDrain := registry.Drain(context.Background()); errDrain != nil {
		t.Fatalf("Drain() error = %v", errDrain)
