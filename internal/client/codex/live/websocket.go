...
func (h *Handler) HandleDirectWebsocket(c *gin.Context) {
	...
	selection, selected, resultCtx, errSelect := h.selectOAuth(ctx, selectionModel, selectionOpts)
	if errSelect != nil {
	...
	dialUpstream := func(current *auth.Auth, confirmDispatch bool) (*liveBootstrapAttempt, *websocket.Conn, *http.Response, error) {
		...
		dialer, errDialer := newProxyAwareSidebandDialer(helpersConfig, current)
		if errDialer != nil {
		...
		if confirmDispatch {
			if errConfirm := h.authManager.ConfirmSelectedAuthDispatch(resultCtx); errConfirm != nil {
				return nil, nil, nil, errConfirm
			}
		}
		attempt := &liveBootstrapAttempt{
			reporter:   helps.NewUsageReporter(ctx, "codex", selectionModel, current),
			selected:   current,
			dispatched: true,
		}
		...
		if response != nil {
			...
			if response.Body != nil {
				attempt.reporter.ObserveResponse(response)
			} else {
				attempt.reporter.MarkFirstResponseByte()
			}
		} else if upstream != nil {
			attempt.statusCode = http.StatusSwitchingProtocols
			attempt.reporter.MarkFirstResponseByte()
		}
		return attempt, upstream, response, errDial
	}

	activeAttempt, upstream, handshakeResponse, errDial := dialUpstream(selected, selection == nil)
	if errDial != nil && selection != nil && handshakeResponse != nil && handshakeResponse.StatusCode == http.StatusUnauthorized {
		...
		if activeAttempt != nil {
			activeAttempt.body = sidebandHandshakeBody(handshakeResponse)
			activeAttempt.err = errDial
			activeAttempt.finalize(ctx, h.authManager, resultCtx, selectionModel, false)
			activeAttempt = nil
		}
		...
		if didRefresh && refreshed != nil {
			selected = refreshed
			logging.SetGinCPATraceID(c, selected.EnsureIndex())
			activeAttempt, upstream, handshakeResponse, errDial = dialUpstream(selected, false)
		}
	}
	if activeAttempt != nil {
		activeAttempt.body = sidebandHandshakeBody(handshakeResponse)
		activeAttempt.err = errDial
		activeAttempt.finalize(ctx, h.authManager, resultCtx, selectionModel, selection == nil)
		activeAttempt = nil
	}
	if errDial != nil {
