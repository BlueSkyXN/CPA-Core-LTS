...
func (h *Handler) HandleHangup(c *gin.Context) {
	...
	if session.homeSelection != nil && session.homeSelection.Active() {
		...
		selection, selectedAuth, selectedResultCtx, errSelect := h.selectOAuth(ctx, session.model, selectionOpts)
		if errSelect != nil {
	...
	performRequest := func(current *auth.Auth, confirmDispatch bool) (*liveBootstrapAttempt, *http.Response, error) {
		...
		attempt := &liveBootstrapAttempt{
			reporter:   helps.NewUsageReporter(ctx, "codex", session.model, current),
			selected:   current,
			dispatched: true,
		}
		...
		return attempt, response, errRequest
	}
	activeAttempt, response, errRequest := performRequest(selected, activeSelection == nil)
	if errRequest != nil {
