...
func TestCodexClientModelsResponse_PreservesUltraReasoningEffort(t *testing.T) {
	...
	for _, rawLevel := range levels {
		...
		if ok && stringModelValue(level, "effort") == "ultra" {
			if got := stringModelValue(level, "description"); got != "Maximum reasoning with automatic task delegation" {
				t.Fatalf("ultra description = %q, want automatic task delegation description", got)
			}
			return
		}
	}
...
func TestCodexClientModelsResponse_GPT56ReasoningLevelsRemainModelSpecific(t *testing.T) {
	resp := BuildResponse([]map[string]any{
		{"id": "gpt-5.6-sol"},
		{"id": "gpt-5.6-terra"},
		{"id": "gpt-5.6-luna"},
	...
	assertCodexClientReasoningEfforts(t, bySlug["gpt-5.6-sol"], []string{"low", "medium", "high", "xhigh", "max", "ultra"})
	assertCodexClientReasoningEfforts(t, bySlug["gpt-5.6-terra"], []string{"low", "medium", "high", "xhigh", "max", "ultra"})
	assertCodexClientReasoningEfforts(t, bySlug["gpt-5.6-luna"], []string{"low", "medium", "high", "xhigh", "max"})
}

func TestCodexClientModelsResponse_SparkKeepsEffortEnabledWithSummaryDefaultOff(t *testing.T) {
	resp := BuildResponse([]map[string]any{{"id": "gpt-5.3-codex-spark"}}, nil, false)
	models, ok := resp["models"].([]map[string]any)
	if !ok || len(models) != 1 {
		t.Fatalf("models = %#v, want one model", resp["models"])
	}
...
func assertCodexClientReasoningEfforts(t *testing.T, model map[string]any, want []string) {
	t.Helper()
	if model == nil {
		t.Fatal("model entry is nil")
	}
	rawLevels, ok := model["supported_reasoning_levels"].([]any)
	if !ok || len(rawLevels) != len(want) {
		t.Fatalf("%s supported_reasoning_levels = %#v, want %v", stringModelValue(model, "slug"), model["supported_reasoning_levels"], want)
	}
	for index, wantEffort := range want {
		level, okLevel := rawLevels[index].(map[string]any)
		if !okLevel || stringModelValue(level, "effort") != wantEffort {
			t.Fatalf("%s supported_reasoning_levels[%d] = %#v, want %q", stringModelValue(model, "slug"), index, rawLevels[index], wantEffort)
		}
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.wantEfforts) == 0 {
				if levels, ok := model["supported_reasoning_levels"].([]any); !ok || levels == nil || len(levels) != 0 {
