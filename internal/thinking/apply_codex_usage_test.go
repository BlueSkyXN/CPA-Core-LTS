...
func TestExtractCodexReasoningEffortWithConfigurationUpdate(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "invalid trailing configuration_update falls back to request baseline",
			provider: "codex",
			model:    "gpt-6-astra",
			body: `{
				"model":"gpt-6-astra",
...
func TestUsageReasoningEffortHonorsCompactionAndInvalidControlBoundaries(t *testing.T) {
	for _, provider := range []string{"codex", "xai", "openai-response"} {
		for _, kind := range []string{"compaction_trigger", "compaction", "context_compaction"} {
			body := []byte(`{"reasoning":{"effort":"high"},"input":[{"type":"configuration_update","reasoning":{"effort":"low"}},{"type":"` + kind + `"}]}`)
			if thinking.ExtractTranslatedReasoningEffort(body, provider) != "high" || thinking.ExtractReasoningEffort(body, provider, "model") != "high" {
				t.Fatalf("%s retained pre-%s effort", provider, kind)
			}
		}
	}
}
