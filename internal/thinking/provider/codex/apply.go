...
//
// Codex models use the reasoning.effort format with model-defined discrete levels.
// This is similar to OpenAI but uses nested field
...
//   - Output format: reasoning.effort (model-supported string level)
...
func (a *Applier) Apply(body []byte, config thinking.ThinkingConfig, modelInfo *registry.ModelInfo) ([]byte, error) {
	...
	if config.Mode == thinking.ModeLevel {
		result, _ := sjson.SetBytes(body, "reasoning.effort", thinking.CanonicalCodexWireEffort(string(config.Level), modelInfo))
		return result, nil
	}
	...
	result, _ := sjson.SetBytes(body, "reasoning.effort", thinking.CanonicalCodexWireEffort(effort, modelInfo))
	return result, nil
}
