...
func TestSanitizeGeminiRequestThoughtSignatures_RejectsUnknownToolCallAndToolResponseSignatures(t *testing.T) {
	// Server-side tool blocks still require a recognized provider envelope.
	...
	if gjson.GetBytes(out, "contents.0.parts.0.thoughtSignature").Exists() {
		t.Fatal("unknown server-tool signature was replayed")
	}
...
func TestSanitizeGeminiRequestThoughtSignatures_RejectsUnknownSnakeCaseToolCallAndResponseSignatures(t *testing.T) {
	// Unknown snake_case signatures must not bypass compatibility checks
	...
	if gjson.GetBytes(out, "contents.0.parts.0.thought_signature").Exists() {
		t.Fatal("unknown server-tool signature was replayed")
	}
	if gjson.GetBytes(out, "contents.0.parts.1.thought_signature").Exists() {
		t.Fatal("unknown server-tool signature was replayed")
	}
}
