...
func TestCodexReasoningReplayAppendMigratesLegacyFlatEntryInHome(t *testing.T) {
	ClearCodexReasoningReplayCache()
	t.Cleanup(ClearCodexReasoningReplayCache)
	client := newFakeCodexReasoningReplayKVClient()
	key := codexReasoningReplayKVKey("gpt-5.4", "session-home-legacy")
	legacyItem := validCodexReasoningReplayItemForTest(21)
	client.values[key] = mustCodexReasoningReplayJSON(t, [][]byte{legacyItem})
	useFakeCodexReasoningReplayKVClient(t, client, true, nil)

	before, found, errGet := GetCodexReasoningReplayItemsRequired(context.Background(), "gpt-5.4", "session-home-legacy")
	if errGet != nil || !found || len(before) != 1 || !bytes.Equal(before[0], legacyItem) {
		t.Fatalf("legacy read before append = %q, found %v, err %v", before, found, errGet)
	}

	newTurn := [][]byte{
		[]byte(`{"type":"` + CodexReasoningReplayTurnType + `","id":"turn-new"}`),
		validCodexReasoningReplayItemForTest(22),
	}
	if !AppendCodexReasoningReplayItemsBestEffort(context.Background(), "gpt-5.4", "session-home-legacy", newTurn) {
		t.Fatal("append to legacy replay entry failed")
	}

	after, found, errGet := GetCodexReasoningReplayItemsRequired(context.Background(), "gpt-5.4", "session-home-legacy")
	if errGet != nil || !found {
		t.Fatalf("legacy read after append = found %v, err %v", found, errGet)
	}
	if len(after) != 4 {
		t.Fatalf("legacy migrated item count = %d, want 4: %q", len(after), after)
	}
	if got := gjson.GetBytes(after[0], "id").String(); got != codexReasoningReplayLegacyTurnID {
		t.Fatalf("legacy marker id = %q, want %q", got, codexReasoningReplayLegacyTurnID)
	}
	if !bytes.Equal(after[1], legacyItem) {
		t.Fatalf("legacy item changed during migration: got %s want %s", after[1], legacyItem)
	}
	if got := gjson.GetBytes(after[2], "id").String(); got != "turn-new" {
		t.Fatalf("new turn id = %q, want turn-new", got)
	}
}
