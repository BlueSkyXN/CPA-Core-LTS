...
const (
	...
	// codexReasoningReplayLegacyTurnID marks flat replay entries written before
	// turn markers were introduced, so appends preserve rather than discard them.
	codexReasoningReplayLegacyTurnID = "cpa-legacy-flat-v1"
...
func appendCodexReasoningReplayTurn(existing, turn [][]byte) [][]byte {
	if len(existing) > 0 && strings.TrimSpace(gjson.GetBytes(existing[0], "type").String()) != CodexReasoningReplayTurnType {
		legacyMarker := []byte(`{"type":"` + CodexReasoningReplayTurnType + `","id":"` + codexReasoningReplayLegacyTurnID + `"}`)
		existing = append([][]byte{legacyMarker}, existing...)
	}
	turnID := ""
