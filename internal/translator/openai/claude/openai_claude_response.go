...
func extractOpenAIUsage(usage gjson.Result) (int64, int64, int64, int64) {
	...
	// OpenAI cache-write details are included in input totals; Claude reports
	// uncached input separately from cache reads and writes.
	if cacheWriteTokens > 0 {
		if inputTokens >= cacheWriteTokens {
