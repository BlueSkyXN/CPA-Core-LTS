...
func TestConvertOpenAIResponsesRequestToCodexKeepsCacheKeyAndStripsUnsupportedCacheControls(t *testing.T) {
	input := []byte(`{
		"model":"gpt-5.6-sol",
	...
	if got := gjson.GetBytes(out, "input.0.content.0.prompt_cache_breakpoint"); got.Exists() {
		t.Fatalf("unsupported prompt_cache_breakpoint was preserved; output=%s", out)
	}
}
